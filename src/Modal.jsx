...
function Modal({ show, onClose }) {
  ...
  function handleAddButton() {
    ...
    setTitle((todosTitle) => [...todosTitle, newtodosTitle]);
    setDesc((todosDescrip) => [...todosDescrip, newtodosDescrip]);
    setPriority((todosPior) => [...todosPior, newtodosP]);
    console.log(title);
    onClose();
  }
        ...
        <div className="bg-white w-[500px] h-[500px] rounded-xl p-5">
          ...
          <form>
            <div className="mb-3">
              ...
              <input
                type="text"
                className="rounded-lg mt-1 w-full border-2 p-1"
                id="title-id"
              ></input>
            </div>
            <div>
              ...
              <textarea
                type="text"
                className="rounded-lg w-full h-[150px] border-2 p-1 mt-1"
                id="description-id"
              ></textarea>
            </div>
            ...
            <div className="pt-[50px] flex flex-row justify-around">
              <button
                className="py-1 px-7 border-2 border-gray-300 rounded-lg"
                onClick={onClose}
              >
                CANCEL
              </button>
              <button
                className="py-1 px-10 bg-blue-400 rounded-lg hover:bg-blue-300"
                type="submit"
